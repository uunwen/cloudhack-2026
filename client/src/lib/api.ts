...
const API_BASE_URL = "https://cloudhack-2026-1.onrender.com";
...
export async function parseContent(input: {
  files: File[];
  text: string;
}): Promise<ParseResult> {
  ...
  if (data && typeof data === "object" && Array.isArray(data.sections)) {
    return {
      sections: data.sections as ParsedSection[],
